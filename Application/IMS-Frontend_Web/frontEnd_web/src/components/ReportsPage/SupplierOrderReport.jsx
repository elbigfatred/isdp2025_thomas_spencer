...
const SupplierOrderReport = () => {
  ...
  useEffect(() => {
    const fetchTxns = async () => {
      try {
        ...
        // Filter txns to include only those where txnType is "Supplier Order"
        const supplierOrderTxns = data.filter(
          (txn) => txn.txnType.txnType === "Supplier Order"
        );

        // Apply date range filter
        const filteredTxns = supplierOrderTxns.filter((txn) => {
  ...
  const handleDownload = () => {
    ...
    link.download = `supplier_order_report.${fileExtension}`;
          ...
          <Select value={txnId} onChange={(e) => setTxnId(e.target.value)}>
            ...
            {txns.map((txn) => (
              <MenuItem key={txn.id} value={txn.id}>
                ID: {txn.id} - Created: {txn.createdDate.split("T")[0]}
              </MenuItem>
            ))}
          </Select>
        </FormControl>
